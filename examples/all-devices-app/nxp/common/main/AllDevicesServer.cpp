...
// Forward declaration - implemented in DeviceAppShell.cpp
 ...
 * is removed from the device. Registered after Server::Init() in the
 * all-devices app (replaces the registration in AppTaskBase::InitServer).
 */
...
#if CHIP_DEVICE_CONFIG_ENABLE_WPA
    gRootNodeDevice = std::make_unique<WifiRootNode>(
        MakeRootNodeContext(initParams, *deviceInfoProvider),
        WifiRootNode::WifiContext{
            .wifiDriver = static_cast<DeviceLayer::NetworkCommissioning::WiFiDriver &>(
                DeviceLayer::NetworkCommissioning::NXPWiFiDriver::GetInstance()),
        });
...
CHIP_ERROR DoStartAllDevicesServer(const std::string & deviceType, bool persistDeviceType)
{
    ...
    ReturnErrorOnFailure(
        Server::GetInstance().GetFabricTable().AddFabricDelegate(&sLastFabricRemovedDelegate));
...
#ifdef ENABLE_CHIP_SHELL
    RegisterDeviceAppShellCommands();
#endif
