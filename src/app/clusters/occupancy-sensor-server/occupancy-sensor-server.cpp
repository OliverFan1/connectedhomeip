...
static bool emitOccupancyChangedEvent(EndpointId ep)
{
	OccupancySensing::Events::OccupancyChanged::Type event;
	BitMask<OccupancySensing::OccupancyBitmap> occState;
    VerifyOrReturnValue(Status::Success == chip::app::Clusters::OccupancySensing::Attributes::Occupancy::Get(ep, &occState), false);
    if(occState.Has(OccupancyBitmap::kOccupied)){
		event.occupancy = 1;
	}
	else{
		event.occupancy = 0;
    ...
    {
        ChipLogError(Zcl, "Unable to emit OccupancyChanged event [ep=%d]", ep);
        return false;
    }

    ChipLogProgress(Zcl, "Emit OccupancyChanged event [ep=%d]", ep);
    return true;
}
...
void halOccupancyStateChangedCallback(EndpointId endpoint, HalOccupancyState occupancyState)
{
	...
	emitOccupancyChangedEvent(endpoint);
}
